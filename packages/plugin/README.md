...
Deploy the field plugin with the CLI. Issue a [personal access token](https://app.storyblok.com/#/me/account?tab=token), rename `.env.local.example` to `.env.local`, open the file, set the value `STORYBLOK_PERSONAL_ACCESS_TOKEN`, and run
